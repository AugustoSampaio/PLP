...
import lf1.plp.functional1.parser.Token;
import plp.debug.core.SourceToken;

/**
 * Relê o código-fonte usando o lexer real de Funcional1 (gerado pelo próprio
...
public final class TokenReader {
	...
	public static List<SourceToken> read(String source) {
		...
		try {
			...
			Func1Parser.ReInit(new ByteArrayInputStream(source.getBytes()));
			Token token = Func1Parser.token_source.getNextToken();
			while (token != null && token.kind != 0) {
				out.add(new SourceToken(token.image, token.beginLine, token.beginColumn,
						token.endLine, token.endColumn));
				token = Func1Parser.token_source.getNextToken();
			}
