...
import li2.plp.imperative2.parser.Token;
import plp.debug.core.SourceToken;

/**
 * Relê o código-fonte usando o lexer real de Imperativa2 (gerado pelo próprio
...
public final class TokenReader {
	...
	public static List<SourceToken> read(String source) {
		...
		try {
			...
			Imp2Parser.ReInit(new ByteArrayInputStream(source.getBytes()));
			Token token = Imp2Parser.token_source.getNextToken();
			while (token != null && token.kind != 0) {
				out.add(new SourceToken(token.image, token.beginLine, token.beginColumn,
						token.endLine, token.endColumn));
				token = Imp2Parser.token_source.getNextToken();
			}
