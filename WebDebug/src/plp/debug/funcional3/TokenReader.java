...
import lf3.plp.functional3.parser.Token;
import plp.debug.core.SourceToken;

/**
 * Relê o código-fonte usando o lexer real de Funcional3 (gerado pelo próprio
...
public final class TokenReader {
	...
	public static List<SourceToken> read(String source) {
		...
		try {
			...
			Func3Parser.ReInit(new ByteArrayInputStream(source.getBytes()));
			Token token = Func3Parser.token_source.getNextToken();
			while (token != null && token.kind != 0) {
				out.add(new SourceToken(token.image, token.beginLine, token.beginColumn,
						token.endLine, token.endColumn));
				token = Func3Parser.token_source.getNextToken();
			}
