...
import plp.functional3.exception.PadraoNaoEncontradoException;
import plp.functional3.exception.TipoParametrosException;
import plp.functional3.exception.TipoRetornoPadraoException;
import plp.functional3.util.AmbienteUtil;
import plp.functional3.util.Padrao;
import plp.functional3.util.PartialInstantiatorVisitor;
...
public class ValorFuncao implements ValorAbstrato {
	...
	public Tipo getTipo(AmbienteCompilacao amb) throws VariavelNaoDeclaradaException,
		...
		// Retorna o tipo do ValorFuncao através do tipo do ValorPadrao
	...
	public boolean checaTipo(AmbienteCompilacao amb)
		...
		for ( DecPadrao decPadrao : this.decPadroes ) {
			...
			// Verifica se a funcão possui o mesmo nome em todos os padrões
			funcaoId = this.verificaNomeFuncao(funcaoId, decPadrao);
			
			// Verifica se os padrões tem a mesma quantidade de expressoes
			numeroExpressoes = this.verificaQuantidadeExpressoes(numeroExpressoes,
					decPadrao);
			
			// Verifica o tipo das expressões
			listaTipoCasamento = this.verificaTipoExpressoes(amb, listaTipoCasamento,
					decPadrao);
			
			// Adiciona o tipo do resultado da expressão valor do padrão
